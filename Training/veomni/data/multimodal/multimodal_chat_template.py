...
class RAEQwenTemplate(ChatTemplate):
    def encode_messages(self, messages: Sequence[Dict[str, str]], max_seq_len: int = 8192) -> Dict[str, List[int]]:
        input_ids, attention_mask, labels = [], [], []
        if "<image>" not in self.tokenizer.added_tokens_encoder:
            self.tokenizer.add_tokens(["<image>"], special_tokens=True)
        if "<image_gen>" not in self.tokenizer.added_tokens_encoder:
            self.tokenizer.add_tokens(["<image_gen>"], special_tokens=True)
        if "<im_start>" not in self.tokenizer.added_tokens_encoder:
            self.tokenizer.add_tokens(["<im_start>"], special_tokens=True)
        if "<im_end>" not in self.tokenizer.added_tokens_encoder:
            self.tokenizer.add_tokens(["<im_end>"], special_tokens=True)
        if "<no_mean>" not in self.tokenizer.added_tokens_encoder:
            self.tokenizer.add_tokens(["<no_mean>"], special_tokens=True)
        image_token_index = self.tokenizer.convert_tokens_to_ids("<image>")
        image_gen_token_id = self.tokenizer.convert_tokens_to_ids("<image_gen>")
        im_gen_start = self.tokenizer.convert_tokens_to_ids("<im_start>")
        im_gen_end = self.tokenizer.convert_tokens_to_ids("<im_end>")
        no_mean = self.tokenizer.convert_tokens_to_ids("<no_mean>")
        im_start = self.tokenizer.convert_tokens_to_ids("<|im_start|>")
        im_end = self.tokenizer.convert_tokens_to_ids("<|im_end|>")
        unmask_tokens_idx =  [198, im_start, im_end, im_gen_start, im_gen_end, no_mean]
        system_prompt = "You are a helpful assistant."

        input_ids += self.tokenizer.apply_chat_template([{"role": "system", "content": system_prompt}])
        labels += [IGNORE_INDEX] * len(input_ids)
        self.tokenizer.chat_template = self.get_jinja_template()
        for message in messages:
            role = message["role"]
            encode_id = self.tokenizer.apply_chat_template([message])
            input_ids += encode_id
            if role in ["user", "system"]:
                labels += [IGNORE_INDEX] * len(encode_id)
            else:
                labels += encode_id
        assert len(input_ids) == len(labels), f"{len(input_ids)} != {len(labels)}"
        for idx, encode_id in enumerate(input_ids):
            if encode_id in unmask_tokens_idx:
                labels[idx] = encode_id
            if encode_id == image_token_index:
                input_ids[idx] = -200
            if encode_id == image_gen_token_id:
                input_ids[idx] = -300
