...
def set_trainable_modules(model, trainable_modules):
    total, trainable = 0, 0
    ...
    if trainable_modules is not None:
        ...
        logger.info(f"🔒 Frozen params: {(total - trainable)/1e6:.2f}M | "
            f"🟢 Trainable params: {trainable/1e6:.2f}M ({100*trainable/total:.2f}%)")
    else:
        for p in model.parameters():
            p.requires_grad = True
        trainable = total
        logger.info(f"✅ Unfrozen all {trainable/1e6:.2f}M parameters.")
