...
def parse_args():
    ...
    if args.model_config is None:
        args.model_config = os.path.join(
            ...
            "./.mim/configs/mask2former/mask2former_swin-s-p4-w7-224_lsj_8x2_50e_coco.py"
        )
    return args
...
def load_models(args):
    ...
    clip_model, _, transform = open_clip.create_model_and_transforms(clip_arch, pretrained="models/ViT-L-14-state.pt", device=DEVICE)
...
def main(args):
    ...
    # 遍历子文件夹（外层进度）
    subfolders = [f for f in os.listdir(args.imagedir) if f.isdigit()]
    for subfolder in tqdm(subfolders, desc="Processing folders"):
        ...
        # 遍历 samples 中的图片（内层进度）
        imagefiles = [f for f in os.listdir(os.path.join(folderpath, "samples")) if re.match(r"\d+\.png", f)]
        for imagename in tqdm(imagefiles, desc=f"Images in {subfolder}", leave=False):
